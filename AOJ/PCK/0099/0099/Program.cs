...
namespace _0099
{
...

    {
        public struct Node : IComparable<Node>
        {
            ...
            public Node(int[] items) { ID = items[0]; CNT = items[1]; }
            public Node(int v1, int v2) { ID = v1; CNT =v2; }
        ...
        public static void Main(string[] args)
        {
            ...
            int[] fish = new int[1000001];
            PriorityQueue<Node> pq = new PriorityQueue<Node>(n);
       
            for (int i = 0 ; i < q ; i++)
            {
                int[] IdAndCnt = RIntAr();
                fish[IdAndCnt[0]] += IdAndCnt[1];
                pq.Enqueue(new Node(IdAndCnt[0], fish[IdAndCnt[0]]));

                while (pq.Count > 0)
                {
                    if (pq.Peek().CNT == fish[pq.Peek().ID])
                    {
                        Console.WriteLine("{0} {1}", pq.Peek().ID, pq.Peek().CNT);
                        break;
                    }
                    pq.Dequeue();
                }                
